        ...
        [
          for alb_key, alb_values in try(port_values.load_balancer, {}) :
          {
            "${service_name}-${container_name}-${port_key}-${alb_key}" = {
              ...
              "name"           = "${local.common_name}-${service_name}-${port_values.container_port}-${alb_key}"
              "port_key"       = port_key
      ...
      [
        ...
        [
          ...
          [
            ...
            {
              "${service_name}-${container_name}-${port_key}-${alb_key}-${rule_key}" = {
    ...
    for service_key, service_config in var.ecs_service_parameters : {
      "${service_key}" = {
        ...
        "${service_key}-${container_key}" => [
          for port_key, port_values in try(container.ports, {}) :
          [
