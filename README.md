...
| skip_destroy                                     | If true, the previous ECS task definition revision is not deregistered when Terraform replaces the task definition. Explicit `null` at the service or defaults level falls back to `true`. | `bool`   | `true`                                                                                                               | no       |
