module "ecs_service" {
  ...
  security_group_egress_rules             = try(each.value.security_group_egress_rules_custom, {}) != {} ? merge(local.security_group_egress_rules[each.key], each.value.security_group_egress_rules_custom) : local.security_group_egress_rules[each.key]
