module "ecs_service" {
  ...
  security_group_ingress_rules            = merge(local.security_group_ingress_rules[each.key], try(each.value.securty_group_ingress_rules_custom, {}))
  security_group_egress_rules             = merge(local.security_group_egress_rules[each.key], try(each.value.security_group_egress_rules_custom, {}))
